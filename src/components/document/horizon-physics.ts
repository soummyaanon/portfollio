...
/** Distance and bearing from the hole to the centre of a box. */
export function seedOf(rect: DOMRect, holeX: number, holeY: number): { r0: number; a0: number } {
  const dx = rect.left + rect.width / 2 - holeX
  const dy = rect.top + rect.height / 2 - holeY
 ...
 *
 * On overflow the walk stops recursing rather than dropping what it has not reached, so an
 * over-full viewport still swallows completely — just in bigger pieces.
 */
const LEAF_HEIGHT = 120
const MAX_SHREDS = 80

/**
 * The elements the hole will take: everything on screen, at the granularity of a row.
 *
 ...
 */
export function collectShreds(root: Element, viewportHeight: number): HTMLElement[] {
